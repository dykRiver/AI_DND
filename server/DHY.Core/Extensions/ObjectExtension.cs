...
public static partial class ObjectExtension
{
    ...
    public static TResult DeserializeJsonResult<TResult>(this object payload)
    {
        if (payload == null)
        {
            return default;
        ...
        {
            return JSON.Deserialize<TResult>(obj.ToString());
        }
        else if (payload is JsonElement jsonElement)
        {
            return JSON.Deserialize<TResult>(jsonElement.GetRawText());
        }
        else if (payload is JArray array)
        {
            return JSON.Deserialize<TResult>(array.ToString());
        }
        else
        {
            if (payload.GetType().IsPrimitive)
            {
                return (TResult)Convert.ChangeType(payload, typeof(TResult));
            }
    ...
    public static object DeserializeJsonResult(this object payload ,Type payloadType)
    {
        if (payload == null)
        {
            return default;
        ...
        if (payload is JObject obj)
        {
            return obj.ToString().ToObject(payloadType);
        }
        else if (payload is JsonElement jsonElement)
        {
            return jsonElement.GetRawText().ToObject(payloadType);
        }
        else if (payload is JArray array)
        {
            return JSON.GetJsonSerializer().Deserialize(array.ToString(), payloadType);
        }
        else
        {
            if (payload.GetType().IsPrimitive)
            {
                return Convert.ChangeType(payload, payloadType);
            }

           throw new NotSupportedException($"can not convert {payload} to type:{payloadType.Name}");
        }
    }
    ...
    /// <returns></returns>
    public static string GetDescription(this object data,string value)
    {
        return data.GetType().GetMember(value.ToString() ?? string.Empty).FirstOrDefault()
