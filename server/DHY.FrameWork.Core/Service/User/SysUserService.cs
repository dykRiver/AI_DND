...
/// 系统用户服务 💥
/// </summary>
[ApiDescriptionSettings(Order = 490, Groups = [CommonConst.SysGroupName])]
public class SysUserService : IDynamicApiController, ITransient
{
    ...
    public virtual async Task<int> SetStatus(UserInput input)
    {
        ...
        {
            _ = sysCacheService.Set($"{CacheConst.KeyBlacklist}{user.Id}", $"{user.RealName}-{user.Phone}");
        ...
        {
            _ = sysCacheService.Remove($"{CacheConst.KeyBlacklist}{user.Id}");
        }
