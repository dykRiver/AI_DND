...
/// 系统租户管理服务 💥
/// </summary>
[ApiDescriptionSettings(Order = 390, Groups = [CommonConst.SysGroupName])]
public class SysTenantService : IDynamicApiController, ITransient
{
    ...
    public async Task CacheTenant(long tenantId = 0)
    {
        ...
        _ = _sysCacheService.Set(CacheConst.KeyTenant, tenantList);
    }
    ...
    public async Task CreateDb(TenantInput input)
    {
        ...
        DataBaseFilter.InitTenantDatabase(App.GetRequiredService<ISqlSugarClient>().AsTenant(), config, new SqlSugarCache());
    }
    ...
    public SqlSugarScopeProvider GetTenantDbConnectionScope(long tenantId)
    {
        ...
        lock (iTenant)
        {
            ...
            SqlSugarSetup.SetDbConfig(tenantConnConfig, new SqlSugarCache());
            DataBaseFilter.SetDbAop(sqlSugarScopeProvider, dbOptions.EnableConsoleSql);
