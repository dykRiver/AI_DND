...
/// </summary>
[AllowAnonymous]
[ApiDescriptionSettings(Order = 150, Groups = [CommonConst.SysGroupName])]
public class SysSmsService : IDynamicApiController, ITransient
{
    ...
    public async Task SendSms([Required] string phoneNumber)
    {
        ...
        if (sendSmsResponse.Body.Code == "OK" && sendSmsResponse.Body.Message == "OK")
        {
            ...
            _ = _sysCacheService.Set($"{CacheConst.KeyPhoneVerCode}{phoneNumber}", verifyCode, TimeSpan.FromSeconds(60));
        }
        else
        {
