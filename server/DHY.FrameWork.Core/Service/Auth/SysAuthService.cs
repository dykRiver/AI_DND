...
/// 系统登录授权服务 💥
/// </summary>
[ApiDescriptionSettings(Order = 500, Groups = [CommonConst.SysGroupName])]
public class SysAuthService : IDynamicApiController, ITransient
{
    ...
    public virtual async Task<LoginOutput> Login([Required] LoginInput input)
    {
        ...
        if (tenant is { Status: StatusEnum.Disable })
        ...
        if (CryptogramUtil.CryptoType == CryptogramEnum.MD5.ToString())
        {
            ...
            {
                _ = _sysCacheService.Set(keyErrorPasswordCount, ++errorPasswordCount, TimeSpan.FromMinutes(30));
        ...
        // 登录成功则清空密码错误次数
        _ = _sysCacheService.Remove(keyErrorPasswordCount);
    ...
    public async Task<int> SwaggerSubmitUrl([FromForm] SpecificationAuth auth)
    {
        try
        {
            _ = _sysCacheService.Set(CommonConst.SysCaptcha, false);
            ...
            _ = _sysCacheService.Remove(CommonConst.SysCaptcha);
