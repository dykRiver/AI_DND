...
/// 系统数据库管理服务 💥
/// </summary>
[ApiDescriptionSettings(Order = 250, Groups = [CommonConst.SysGroupName])]
public class SysDatabaseService : IDynamicApiController, ITransient
{
    ...
    {
        var config = App.GetOptions<DbConnectionOptions>().ConnectionConfigs.FirstOrDefault(u => u.ConfigId.ToString() == input.ConfigId);
        input.Position = string.IsNullOrWhiteSpace(input.Position) ? "DHY.Application" : input.Position;
    ...
    {
        var config = App.GetOptions<DbConnectionOptions>().ConnectionConfigs.FirstOrDefault(u => u.ConfigId.ToString() == input.ConfigId);
        input.Position = string.IsNullOrWhiteSpace(input.Position) ? "DHY.Core" : input.Position;
        ...
        if (orderField != null)
            _ = query.OrderBy(orderField.DbColumnName);
        // 其次用Id排序
