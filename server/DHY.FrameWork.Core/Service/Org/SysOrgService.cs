...
/// 系统机构服务 💥
/// </summary>
[ApiDescriptionSettings(Order = 470, Groups = [CommonConst.SysGroupName])]
public class SysOrgService : IDynamicApiController, ITransient
{
    ...
    public async Task<List<long>> GetUserOrgIdList()
    {
        ...
        if (orgIdList == null || orgIdList.Count < 1)
        {
            ...
            _ = _sysCacheService.Set($"{CacheConst.KeyUserOrg}{userId}", orgIdList, TimeSpan.FromDays(7)); // 存缓存
        }
        return orgIdList;
    }
    ...
    private async Task<List<long>> GetUserOrgIdList(List<SysRole> roleList)
    {
        ...
        // 缓存当前用户最大角色数据范围
        _ = _sysCacheService.Set(CacheConst.KeyRoleMaxDataScope + _userManager.UserId, strongerDataScopeType, TimeSpan.FromDays(7));
