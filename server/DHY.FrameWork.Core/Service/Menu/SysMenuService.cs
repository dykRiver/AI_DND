...
/// 系统菜单服务 💥
/// </summary>
[ApiDescriptionSettings(Order = 450, Groups = [CommonConst.SysGroupName])]
public class SysMenuService : IDynamicApiController, ITransient
{
    ...
    /// <returns></returns>
    [DisplayName("根据菜单ID获取菜单项")]
    public async Task<SysMenu> GetById(DeleteMenuInput input)
    {
        return await _sysMenuRep.GetByIdAsync(input.Id);
    }
    ...
    [DisplayName("增加菜单")]
    public async Task<SysMenu> AddMenu(AddMenuInput input)
    {
        ...
        var newMenuItem = await _sysMenuRep.AsInsertable(sysMenu).ExecuteReturnEntityAsync();
        // 清除缓存
        DeleteMenuCache();
    ...
    public async Task<List<string>> GetOwnBtnPermList()
    {
        ...
        if (permissions == null)
        {
            ...
            permissions = menuIdList.Count > 0 || _userManager.SuperAdmin
                ...
                : new List<string>();
            _ = _sysCacheService.Set(CacheConst.KeyUserButton + userId, permissions, TimeSpan.FromDays(7));
        }
    ...
    public async Task<List<string>> GetAllBtnPermList()
    {
        ...
        if (permissions == null || permissions.Count == 0)
        {
            permissions = await _sysMenuRep.AsQueryable()
                .Where(u => u.Type == MenuTypeEnum.Btn)
                .Select(u => u.Permission).ToListAsync();
            _ = _sysCacheService.Set(CacheConst.KeyUserButton + 0, permissions);
        }
