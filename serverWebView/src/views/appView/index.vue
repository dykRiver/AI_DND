<template>
  <div class="sys-formDes-container" style="height: 100%;">
...
const handleQuery = async () => {
  ...
  let urlQuery = router.currentRoute.value.query
  ...
  vFormRef.value.setFormJson(JSON.parse(String(res.data.result?.templateConfig)));
...
//   vFormRef.value.globalDsv.evetHub = evetHub;
  vFormRef.value.globalDsv.baseUrl = import.meta.env.VITE_API_URL;
    vFormRef.value.globalDsv.prescriptionNo = urlQuery.prescriptionNo;
//   const accessToken = Local.get(accessTokenKey);
  // 获取刷新 token
//   const refreshAccessToken = Local.get(refreshAccessTokenKey);
//   vFormRef.value.globalDsv.token = `Bearer ${accessToken}`;
//   vFormRef.value.globalDsv.xtoken = `Bearer ${refreshAccessToken}`;
...
onMounted(async () => {
  handleQuery();
//   vFormRef.value.setFormJson(formJson1);
});
